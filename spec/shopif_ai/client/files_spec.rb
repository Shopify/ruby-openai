  ...
  describe "#files", :vcr do
    ...
    let(:upload) do
      VCR.use_cassette(upload_cassette) do
        ShopifAi::Client.new.files.upload(parameters: { file: file, purpose: upload_purpose })
      end
    end
    ...
    describe "#upload" do
      ...
      context "with an invalid purpose" do
        ...
        it "logs a warning" do
          ...
          expected_message += "purpose list: #{ShopifAi::Files::PURPOSES.join(', ')}."

          expect(ShopifAi).to receive(:log_message)
    ...
    describe "#list" do
      ...
      let(:response) { ShopifAi::Client.new.files.list(parameters: { purpose: "fine-tune" }) }
    ...
    describe "#retrieve" do
      ...
      let(:upload_cassette) { "#{cassette} upload" }
      let(:response) { ShopifAi::Client.new.files.retrieve(id: upload_id, parameters: {}) }
    ...
    describe "#content" do
      ...
      let(:upload_cassette) { "#{cassette} upload" }
      let(:response) { ShopifAi::Client.new.files.content(id: upload_id, parameters: {}) }
    ...
    describe "#delete" do
      ...
      let(:response) do
        ShopifAi::Client.new.files.delete(id: upload_id)
      end
      ...
      before do
        ...
        retrieved = VCR.use_cassette(retrieve_cassette) do
          ShopifAi::Client.new.files.retrieve(id: upload_id)
        end
        tries = 0
    ...
    describe "#fetch_image" do
      ...
      let(:file) { File.join(RSPEC_ROOT, "fixtures/files", filename) }
      let(:upload_purpose) { "vision" }
      let(:response) { ShopifAi::Client.new.files.content(id: upload_id) }
