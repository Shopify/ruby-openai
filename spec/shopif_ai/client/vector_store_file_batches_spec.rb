  ...
  describe "#vector_store_file_batches" do
    let(:vector_store_id) do
      VCR.use_cassette("#{cassette} vector_store setup") do
        ShopifAi::Client.new.vector_stores.create(parameters: {})["id"]
      end
    end
    ...
    let(:file_id) do
      VCR.use_cassette("#{cassette} file setup") do
        ShopifAi::Client.new.files.upload(parameters: { file: file, purpose: upload_purpose })["id"]
      end
    end

    let(:file_batch_id) do
      VCR.use_cassette("#{cassette} vector_store_file_batch setup") do
        ShopifAi::Client.new.vector_store_file_batches.create(
    ...
    describe "#list" do
      ...
      let(:response) do
        ShopifAi::Client.new.vector_store_file_batches.list(vector_store_id: vector_store_id,
                                                          id: file_batch_id)
    ...
    describe "#retrieve" do
      ...
      let(:response) do
        ShopifAi::Client.new.vector_store_file_batches.retrieve(vector_store_id: vector_store_id,
                                                              id: file_batch_id)
      end
    ...
    describe "#cancel" do
      ...
      let(:response) do
        ShopifAi::Client.new.vector_store_file_batches.cancel(vector_store_id: vector_store_id,
                                                            id: file_batch_id)
      end
